...
class VersioningTest {
    ...
    fun `second foundation bumps the major`() {
        assertEquals(
            "v2.0",
            Versioning.nextVersion(listOf("v1.0", "v1.1"), BaseKind.FOUNDATION, null)
        )
    }
    ...
    fun `continuation bumps the minor within the parent major`() {
        ...
        assertEquals(
            "v1.2",
            Versioning.nextVersion(listOf("v1.0", "v1.1", "v2.0"), BaseKind.CONTINUATION, "v1.1")
        )
        assertEquals(
            "v2.1",
            Versioning.nextVersion(listOf("v1.0", "v2.0"), BaseKind.CONTINUATION, "v2.0")
        )
    }
    ...
    fun `serve command embeds checkpoint and served name`() {
        val v =
            ModelVersion(
                ...
                baseKind = BaseKind.CONTINUATION,
