...
class ContentsPartsSerializerTest {
    ...
    fun `serializes a tool-calling conversation to contents-parts`() {
        val example = SftExample(
            id = "x1",
            turns = listOf(
                Turn(TurnRole.USER, TurnKind.TEXT, text = "Show me electricians near Sakinaka"),
                Turn(TurnRole.MODEL, TurnKind.TOOL_CALL, toolName = "search_workers", argsJson = """{"skill":"electrician","location":"Sakinaka"}"""),
                Turn(TurnRole.USER, TurnKind.TOOL_RESPONSE, toolName = "search_workers", resultJson = """{"results":[{"id":"w12","name":"Ramesh"}]}"""),
        ...
        @Suppress("UNCHECKED_CAST")
        val functionCall = callPart["functionCall"] as Map<String, Any?>
        assertEquals("search_workers", functionCall["name"])
        @Suppress("UNCHECKED_CAST")
        val args = functionCall["args"] as Map<String, Any?>
    ...
    fun `validator flags a tool call not followed by a response`() {
        val bad = SftExample(
            ...
            turns = listOf(
                ...
                Turn(TurnRole.MODEL, TurnKind.TOOL_CALL, toolName = "list_skills", argsJson = "{}"),
            ),
        )
