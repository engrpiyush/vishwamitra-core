...
class DpoSerializerTest {
    ...
    private fun pair(chosen: String, rejected: String) =
        DpoPair(
            id = "d1",
            promptTurns =
                listOf(
                    Turn(
                        TurnRole.USER,
                        TurnKind.TEXT,
                        text = "Painting ke liye banda chahiye, kitna charge?"
                    )
                ),
            chosenText = chosen,
    ...
    fun `serializes prompt contents plus chosen and rejected`() {
        val out =
            serializer.toPreference(
                pair("Area bata do, quote nikalta hoon.", "I cannot help with that.")
            )

        @Suppress("UNCHECKED_CAST") val contents = out["contents"] as List<Map<String, Any?>>
        assertEquals("user", contents[0]["role"])

        @Suppress("UNCHECKED_CAST") val chosen = out["chosen"] as Map<String, Any?>
    ...
    fun `validator flags identical and empty candidates`() {
        ...
        assertTrue(
            validator.validate(pair("ok", "")).any { it.contains("Rejected response is empty") }
        )
    }
