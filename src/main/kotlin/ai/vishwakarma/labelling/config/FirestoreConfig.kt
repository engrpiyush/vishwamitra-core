...
class FirestoreConfig {
    ...
    fun firestore(props: AppProperties): Firestore {
        ...
        if (!emulator.isNullOrBlank()) {
            log.info(
                "Firestore: using emulator at {} (db={})",
                emulator,
                props.gcp.firestoreDatabase
            )
            // The emulator speaks plaintext h2c — wire an explicit plaintext channel + no
            // credentials.
            builder
            ...
            try {
                ...
                log.info(
                    "Firestore: using Application Default Credentials (db={})",
                    props.gcp.firestoreDatabase
                )
            } catch (e: IOException) {
                log.warn(
                    "Firestore: no ADC found ({}); RPCs will fail until credentials or emulator are configured",
                    e.message
                )
