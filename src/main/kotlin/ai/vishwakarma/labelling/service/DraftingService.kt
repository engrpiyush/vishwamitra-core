...
/**
 * Selects an available [DraftingProvider] (in providers-catalog order) and runs the requested
 * draft. Any missing-config or runtime error degrades to manual: callers get a Left and keep
 * editing by hand (this is the "ManualOnly" fallback referenced in the plan — no separate bean
 * needed).
 */
@Service
class DraftingService(
    ...
    fun activeProvider(): DraftingProvider? =
        providers
            .filter { it.available() }
            .minByOrNull {
                providerService.knownProviders.indexOf(it.id).let { i ->
    ...
    fun draftConversation(scenario: Scenario?, tags: ExampleTags): Either<DomainError, List<Turn>> =
        run {
            activeProvider()?.let { p ->
                attempt(p.id) { p.draftConversation(scenario, tags, tools()) }
            } ?: manual()
        }

    fun draftNextTurn(turns: List<Turn>): Either<DomainError, Turn> = run {
        activeProvider()?.let { p -> attempt(p.id) { p.draftNextTurn(turns, tools()) } } ?: manual()
    }

    fun draftTwoCandidates(promptTurns: List<Turn>): Either<DomainError, CandidatePair> = run {
        activeProvider()?.let { p -> attempt(p.id) { p.draftTwoCandidates(promptTurns, tools()) } }
            ?: manual()
    }
    ...
    private fun <T> attempt(providerId: String, block: () -> T): Either<DomainError, T> =
        runCatching { block().right() as Either<DomainError, T> }
            .getOrElse { e ->
                ...
                DomainError.Invalid("Drafting via $providerId failed (${e.message}); edit manually")
                    .left()
            }
