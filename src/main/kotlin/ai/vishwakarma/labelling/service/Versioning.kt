...
object Versioning {
     ...
     * @param baseKind FOUNDATION → new major `v{maxMajor+1}.0`; CONTINUATION →
     *   `v{parentMajor}.{maxMinorInMajor+1}`
    ...
    fun nextVersion(existing: List<String>, baseKind: BaseKind, parentVersion: String?): String {
        ...
        return when (baseKind) {
            ...
            BaseKind.CONTINUATION -> {
                val parentMajor =
                    parse(parentVersion ?: error("continuation needs a parent version")).first
                val maxMinor =
                    parsed.filter { it.first == parentMajor }.maxOfOrNull { it.second } ?: 0
