...
class SftService(
        ...
        mutate(id) { ex ->
            val turn =
                when (kind) {
                    ...
                    TurnKind.TOOL_CALL ->
                        Turn(role = TurnRole.MODEL, kind = kind, toolName = "", argsJson = "{}")
                    TurnKind.TOOL_RESPONSE ->
                        Turn(role = TurnRole.USER, kind = kind, toolName = "", resultJson = "{}")
                }
    ...
    fun moveTurn(id: String, index: Int, delta: Int): Either<DomainError, SftExample> =
        mutate(id) { ex ->
            val turns = ex.turns.toMutableList()
            val target = index + delta
            if (index in turns.indices && target in turns.indices) {
                val tmp = turns[index]
                turns[index] = turns[target]
                turns[target] = tmp
            }
            ex.copy(turns = turns)
        }
    ...
    fun approve(id: String, actor: String?): Either<DomainError, SftExample> {
        ...
        if (ex.status != ExampleStatus.SUBMITTED)
            return DomainError.Invalid("Only submitted examples can be approved").left()
    ...
    fun sendBack(id: String, actor: String?, comment: String): Either<DomainError, SftExample> {
        val ex = repo.findById(id) ?: return DomainError.NotFound("Example $id not found").left()
        if (ex.status != ExampleStatus.SUBMITTED)
            return DomainError.Invalid("Only submitted examples can be sent back").left()
        if (comment.isBlank())
    ...
    private fun persist(ex: SftExample): SftExample {
        ...
        val updated =
            ex.copy(tags = ex.tags.copy(hasToolCall = hasToolCall), updatedAt = Instant.now())
        repo.save(updated)
        return updated
    }
}
