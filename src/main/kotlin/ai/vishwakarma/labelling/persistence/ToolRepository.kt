...
class ToolRepository(private val db: Firestore) {
    ...
    fun findAll(): List<Tool> =
        col.get().await().documents.map { it.toTool() }.sortedBy { it.name }
    ...
    private fun Tool.toMap(): Map<String, Any?> = mapOf(
        "name" to name,
        "description" to description,
        "params" to params.map { mapOf("name" to it.name, "type" to it.type, "required" to it.required, "desc" to it.desc) },
    ...
    private fun DocumentSnapshot.toTool(): Tool = Tool(
        id = id,
        name = getString("name") ?: "",
        description = getString("description") ?: "",
        params = (get("params") as? List<Map<String, Any?>> ?: emptyList()).map {
            ToolParam(
        ...
        status = runCatching { ToolStatus.valueOf(getString("status") ?: "ACTIVE") }.getOrDefault(ToolStatus.ACTIVE),
