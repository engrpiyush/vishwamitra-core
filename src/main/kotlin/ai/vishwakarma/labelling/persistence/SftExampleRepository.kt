...
class SftExampleRepository(private val db: Firestore) {
    ...
    fun findAll(): List<SftExample> =
        col.orderBy("updatedAt", Query.Direction.DESCENDING).get().await().documents.map { it.toExample() }
    ...
    private fun SftExample.toMap(): Map<String, Any?> = mapOf(
        "tags" to mapOf(
            "skill" to tags.skill,
        ...
        "reviewComments" to reviewComments.map {
    ...
    private fun DocumentSnapshot.toExample(): SftExample {
        ...
        return SftExample(
            ...
            source = runCatching { ExampleSource.valueOf(getString("source") ?: "MANUAL") }.getOrDefault(ExampleSource.MANUAL),
            ...
            reviewComments = commentsList.map {
                ReviewComment(
                    by = it["by"] as? String,
                    text = it["text"] as? String ?: "",
                    at = (it["at"] as? com.google.cloud.Timestamp)?.let { ts -> Instant.ofEpochSecond(ts.seconds, ts.nanos.toLong()) } ?: Instant.now(),
                )
            },
