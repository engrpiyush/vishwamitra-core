...
class ModelVersionRepository(private val db: Firestore) {
    ...
    fun findAll(): List<ModelVersion> = col.get().await().documents.map { it.toVersion() }

    fun findByFamily(family: String): List<ModelVersion> = findAll().filter { it.family == family }

    fun save(version: ModelVersion) {
        col.document(version.id).set(version.toMap()).await()
    }

    private fun ModelVersion.toMap(): Map<String, Any?> =
        mapOf(
    ...
    private fun DocumentSnapshot.toVersion(): ModelVersion =
        ModelVersion(
            id = id,
            ...
            method =
                runCatching { TuningMethod.valueOf(getString("method") ?: "SFT") }
                    .getOrDefault(TuningMethod.SFT),
            baseKind =
                runCatching { BaseKind.valueOf(getString("baseKind") ?: "FOUNDATION") }
                    .getOrDefault(BaseKind.FOUNDATION),
            ...
            status =
                runCatching { VersionStatus.valueOf(getString("status") ?: "TRAINING") }
                    .getOrDefault(VersionStatus.TRAINING),
            promotion =
                runCatching { Promotion.valueOf(getString("promotion") ?: "NONE") }
                    .getOrDefault(Promotion.NONE),
